...
class NetworkController<T: StarwarsAPI> {
   ...
   static func fetchAllCharacters(completion: @escaping ([T]?, Error?) -> Void) {
        ...
        URLSession.shared.dataTask(with: url) { (data, response, error) in
            ...
            do {
                //pagedResponse has the results property in it that we need to drill down in the api
                let arrayOfCharacters = try decoder.decode(PagedResponse<T>.self, from: data).results
